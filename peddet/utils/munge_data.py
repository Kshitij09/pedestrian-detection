...
parser.add_argument(
    "-n", dest="fname", type=str, help="Output filename", default="data"
)
...
def f_standing(x: str):
    return x.endswith("Standing")


def f_walking(x: str):
    return x.endswith("Walking")
...
def create_csv(target_dir: str, output: str):
    ...
    dataframe = dataframe.reindex(
        columns=["id", "filename", "label", "width", "height", "box"]
    )
...
def perform_split(source_fname: str):
    ...
    final_df = final_df.reindex(
        columns=["id", "filename", "label", "width", "height", "area", "box", "fold"]
    )
